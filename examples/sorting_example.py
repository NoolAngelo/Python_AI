...
def benchmark_sort(sort_func: Callable[[List[int]], List[int]], data: List[int], name: str) -> None:
    """
    ...
    # Print results
    print(f"{name:15} | Time: {end - start:.6f} seconds | Sorted correctly: {is_sorted(result)}")
...
def main() -> None:
    ...
    for size in data_sizes:
        ...
        print(f"{'Python sorted()':15} | Time: {end - start:.6f} seconds | Sorted correctly: {is_sorted(sorted_data)}")
